...
router.put("/user/:id", async (req: Request<{ id: string }, {}, User>, res: Response<User | { error: string }>, next: NextFunction): Promise<void> => {
  ...
  try {
    const newUser = await pool.query('UPDATE "user" SET user_name = $1, email = $2 WHERE user_id = $3 RETURNING*', [user_name, email, id]);
    res.json(newUser.rows[0]);
  }
  catch (error: unknown) {
    next(error);
...
router.patch("/user/:id", async (req: Request<{ id: string }, {}, User>, res: Response<User | { error: string }>, next: NextFunction): Promise<void> => {
  const { params: { id } } = req;

  // Eftersom vi har types som ser till att bodyn är korrekt formaterad när den skickas in så kan vi spreada denna på rad 105 istället för att destructa
  ...
  try {
    ...
    let updated_user = { ...result.rows[0], ...req.body };

    const newUser = await pool.query('UPDATE "user" SET user_name = $1, email = $2 WHERE user_id = $3 RETURNING*', [updated_user.user_name, updated_user.email, updated_user.id]);
    res.json(newUser.rows[0]);
  }
  catch (error: unknown) {
    next(error);
...
router.delete("/user/:id", async (req: Request<{ id: string }, {}, User>, res: Response<User | { error: string }>, next: NextFunction): Promise<void> => {
  const { params: { id }, } = req;

  try {
    const newUser = await pool.query('DELETE FROM "user" WHERE user_id = $1 RETURNING *');
    res.json(newUser.rows[0]);
  }
  catch (error: unknown) {
    next(error);
