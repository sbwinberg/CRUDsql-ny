...
router.get("/spec1", async (req: Request, res: Response<User[] | User[] | { error: string }>, next: NextFunction): Promise<void> => {
  try {
...
router.get("/spec2", async (req: Request, res: Response<User[] | User[] | { error: string }>, next: NextFunction): Promise<void> => {
  try {
    const postCounter = await pool.query(
      `SELECT "user".user_name, COUNT(post.post_user_id) AS number_of_posts
          FROM "user"
...
router.get("/spec3", async (req: Request, res: Response<User[] | User[] | { error: string }>, next: NextFunction): Promise<void> => {
  try {
    const postCounter = await pool.query(
